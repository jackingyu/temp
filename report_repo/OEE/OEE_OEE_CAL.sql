 ...
 BEGIN
	...
	*/
	sql_select_all_shift = SELECT
				...
				L.END_DATE_TIME,
				L.RESOURCE_STATE,
				RC.REASON_CODE_GROUP,
				S.DESCRIPTION,
				L.REASON_CODE,
				S.SHIFT
				FROM MHP.WORK_CENTER_SHIFT AS S
				INNER JOIN MHP.WORK_CENTER AS C ON C.HANDLE=S.WORK_CENTER_BO
				INNER JOIN MHP.RESRCE AS R ON R.SITE=C.SITE AND R.WORK_AREA=C.WORK_CENTER
				INNER JOIN MHP.RESOURCE_REASON_CODE_LOG AS L ON L.SITE=R.SITE AND L.RESOURCE_BO=R.HANDLE
				INNER JOIN MHP.REASON_CODE AS RC ON RC.SITE=L.SITE AND RC.REASON_CODE=L.REASON_CODE
				WHERE L.SITE=:site
			 	AND L.RESOURCE_BO IN (:resourceBo)
			 	AND CAST(L.START_DATE_TIME AS DATE) >= :startDate
			 	AND CAST(L.START_DATE_TIME AS DATE) <= :endDate;
	...
	var_out = SELECT
				BY_TIME,
				CONCAT(ROUND(A,2), '%') AS A,
				CONCAT(ROUND(P,2), '%') AS P,
				CONCAT(ROUND(Q,2), '%') AS Q,
				CONCAT(ROUND(OEE,2), '%') AS OEE,
				1 AS QTY FROM :sql_oee_average;
